...
from wtforms import StringField, TextAreaField, DateTimeField, FloatField, IntegerField, SelectField
...
class RaceForm(FlaskForm):
    ...
    date = DateTimeField('Race Date', format='%Y-%m-%dT%H:%M', validators=[DataRequired()])  # Changed format
    location = StringField('Location', validators=[DataRequired()])
    description = TextAreaField('Description')
    registration_fields = SelectMultipleField('Registration Fields',
        choices=[
...
class EventForm(FlaskForm):
    ...
    number_of_laps = IntegerField('Number of Laps', validators=[Optional(), NumberRange(min=1)])
...
class RaceResultForm(FlaskForm):
    bib_number = IntegerField('Bib Number', validators=[DataRequired()])
